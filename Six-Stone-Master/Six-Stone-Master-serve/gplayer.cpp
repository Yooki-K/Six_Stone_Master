...
GPlayer::GPlayer(bool flag,QObject *parent) : QObject(parent)
{
    myflag=flag;
...
void GPlayer::myturn(Gamemodel *game,int x,int y){
    ...
    if(game->type!=MM){
        if(myflag) {
            if(game->AItype==easy)     calculatBlack(game,x,y);
            if(game->AItype==diff)     calculatBlackUp(game,x,y);
        }
        else{
            if(game->AItype==easy)     calculatWhite(game,x,y);
            if(game->AItype==diff)     calculatWhiteUp(game,x,y);
    }//计算此子
...
void GPlayer::calculatBlack(Gamemodel *game,int fx, int fy)
{
    int i=1;
    while(1){
...
void GPlayer::calculatWhite(Gamemodel *game,int fx, int fy)
{
    int i=1;
    while(1){
        if(!isRanged(fy-i))  break;
...
void GPlayer::calculatWhiteUp(Gamemodel *game,int fx, int fy)
{
    struct after
    {
        bool notfirst=0;
...
void GPlayer::calculatBlackUp(Gamemodel *game,int fx, int fy)
{
...
maxscore GPlayer::maxwhite(Gamemodel *game){
        maxscore scoremax;
...
maxscore GPlayer::maxblack(Gamemodel *game)
{
    maxscore scoremax;
    for(int i=0;i<columnline;i++)
    {
